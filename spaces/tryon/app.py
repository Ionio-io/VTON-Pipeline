"""
Z-Image Turbo Virtual Try-On  --  Hugging Face Space
====================================================
Gradio application that runs Z-Image Turbo image-to-image inference
to generate virtual try-on results from a base model image and a
text description of the garment.

Deploy:
    1. Create a new Space at https://huggingface.co/new-space
       - SDK: Gradio
       - Hardware: T4-small (16 GB VRAM, ~$0.40/hr) or A10G (faster)
    2. Upload this app.py and requirements.txt to the Space root.
    3. The Space will install dependencies and start automatically.

Programmatic API:
    from gradio_client import Client

    client = Client("YOUR_USERNAME/YOUR_SPACE_NAME")
    result = client.predict(
        base_model_image,       # PIL Image or local file path
        "Navy Blue Slim-Fit Shirt",
        "Cotton, button-down collar, relaxed fit",
        "man",
        "Rectangle",
        "Medium / Olive",
        0.55,                   # strength
        42,                     # seed
        api_name="/predict",
    )
    output_image = result[0]
"""

import time

import gradio as gr
import torch
from diffusers import ZImageImg2ImgPipeline
from PIL import Image


# ---------------------------------------------------------------------------
# Model initialisation  (runs once at Space startup)
# ---------------------------------------------------------------------------

MODEL_REPO = "Tongyi-MAI/Z-Image-Turbo"
DEVICE     = "cuda" if torch.cuda.is_available() else "cpu"
DTYPE      = torch.bfloat16 if DEVICE != "cpu" else torch.float32

print(f"Loading {MODEL_REPO} on {DEVICE} ...")
_t0   = time.time()
pipe  = ZImageImg2ImgPipeline.from_pretrained(MODEL_REPO, torch_dtype=DTYPE)
pipe  = pipe.to(DEVICE)
print(f"Model ready in {time.time() - _t0:.1f}s")


# ---------------------------------------------------------------------------
# Constants
# ---------------------------------------------------------------------------

# Turbo inference settings -- guidance_scale must stay 0.0
NUM_STEPS      = 9
GUIDANCE_SCALE = 0.0

IMG_SIZE = (768, 1024)   # (width, height) portrait aspect ratio

NEGATIVE_PROMPT = (
    "deformed, extra limbs, blurry, low quality, watermark, text, logo, "
    "artifacts, bad anatomy, distorted clothing, ugly, duplicate"
)

BODY_TYPES = [
    "Rectangle",
    "Inverted Trapezoid",
    "Triangle",
    "Oval",
    "Trapezoid",
    "Hourglass",
    "Sporty",
]

SKIN_TONES = [
    "Medium / Olive",
    "Light Brown",
    "Dark Brown",
]


# ---------------------------------------------------------------------------
# Prompt construction
# ---------------------------------------------------------------------------

def build_prompt(
    garment_title:  str,
    garment_detail: str,
    gender:         str,
    body_type:      str,
    skin_tone:      str,
) -> str:
    """
    Build the try-on prompt from garment and model attributes.

    Args:
        garment_title:  Short product name (e.g. "Navy Blue Relaxed-Fit Jeans").
        garment_detail: Optional fabric/fit description (e.g. "Cotton, slim fit").
        gender:         "man" or "woman".
        body_type:      Body shape from BODY_TYPES list.
        skin_tone:      Skin tone from SKIN_TONES list.

    Returns:
        Complete prompt string for the diffusion model.
    """
    detail_clause = f" {garment_detail.strip()}." if garment_detail.strip() else ""
    return (
        f"Full-body studio photograph of an Indian {gender} "
        f"with {skin_tone} skin tone and {body_type} body type, "
        f"wearing {garment_title}.{detail_clause} "
        "White seamless studio background, soft diffused overhead lighting, "
        "sharp full-body focus, professional e-commerce fashion photography, 4K."
    )


# ---------------------------------------------------------------------------
# Inference
# ---------------------------------------------------------------------------

def run_tryon(
    model_image:    Image.Image,
    garment_title:  str,
    garment_detail: str,
    gender:         str,
    body_type:      str,
    skin_tone:      str,
    strength:       float,
    seed:           int,
) -> tuple:
    """
    Run Z-Image Turbo img2img and return the output image with a status string.

    Args:
        model_image:    Base model photo (uploaded by user).
        garment_title:  Name of the garment to dress the model in.
        garment_detail: Optional extra garment description.
        gender:         "man" or "woman".
        body_type:      Body shape string.
        skin_tone:      Skin tone string.
        strength:       Edit strength (0.35-0.80; lower = more original preserved).
        seed:           Random seed.

    Returns:
        Tuple of (output PIL Image, status info string).
    """
    if model_image is None:
        raise gr.Error("Please upload a base model image.")
    if not garment_title.strip():
        raise gr.Error("Please enter a garment title.")

    init = model_image.convert("RGB").resize(IMG_SIZE, Image.LANCZOS)
    prompt    = build_prompt(garment_title, garment_detail, gender, body_type, skin_tone)
    generator = torch.Generator(device=DEVICE).manual_seed(int(seed))

    t0 = time.time()
    output = pipe(
        prompt              = prompt,
        image               = init,
        strength            = float(strength),
        num_inference_steps = NUM_STEPS,
        guidance_scale      = GUIDANCE_SCALE,
        negative_prompt     = NEGATIVE_PROMPT,
        generator           = generator,
    ).images[0]
    elapsed = time.time() - t0

    info = (
        f"Generated in {elapsed:.1f}s  |  strength={strength}  |  seed={seed}\n"
        f"Prompt: {prompt}"
    )
    return output, info


# ---------------------------------------------------------------------------
# Gradio UI
# ---------------------------------------------------------------------------

with gr.Blocks(title="Z-Image Turbo -- Virtual Try-On", theme=gr.themes.Soft()) as demo:

    gr.Markdown(
        """
        ## Z-Image Turbo -- Virtual Try-On

        Upload a **base model image**, describe the garment, and generate a
        try-on result in ~2-3 seconds on GPU.

        - Use **strength 0.45-0.60** to preserve the model's face and body.
        - Higher strength = bigger edit but may distort the model identity.
        """
    )

    with gr.Row():

        with gr.Column(scale=1):
            model_image = gr.Image(
                label="Base Model Image",
                type="pil",
                height=400,
            )
            gr.Markdown("*Upload one of the pre-generated `generated_images/*.png` files.*")

        with gr.Column(scale=1):
            garment_title = gr.Textbox(
                label="Garment Title",
                placeholder="e.g. Navy Blue Relaxed-Fit Mid-Rise Jeans",
            )
            garment_detail = gr.Textbox(
                label="Garment Detail  (optional)",
                placeholder="e.g. Cotton fabric, slim fit, button closure",
                lines=2,
            )
            gender = gr.Radio(
                choices=["man", "woman"],
                value="man",
                label="Gender",
            )
            body_type = gr.Dropdown(
                choices=BODY_TYPES,
                value="Rectangle",
                label="Body Type",
            )
            skin_tone = gr.Dropdown(
                choices=SKIN_TONES,
                value="Medium / Olive",
                label="Skin Tone",
            )
            with gr.Row():
                strength = gr.Slider(
                    minimum=0.35, maximum=0.80, value=0.55, step=0.05,
                    label="Strength  (lower = more original preserved)",
                )
                seed = gr.Number(value=42, label="Seed", precision=0)

            generate_btn = gr.Button("Generate Try-On", variant="primary")

    with gr.Row():
        output_image = gr.Image(label="Try-On Result", height=500)
        status_box   = gr.Textbox(label="Status", lines=4, interactive=False)

    generate_btn.click(
        fn=run_tryon,
        inputs=[
            model_image, garment_title, garment_detail,
            gender, body_type, skin_tone, strength, seed,
        ],
        outputs=[output_image, status_box],
        api_name="predict",
    )

    gr.Examples(
        examples=[
            [
                None,
                "Dark Brown Relaxed-Fit Mid-Rise Jeans",
                "Smooth cotton fabric, skater fit, mid-rise waist, multiple pockets",
                "man", "Rectangle", "Medium / Olive", 0.55, 42,
            ],
            [
                None,
                "Pink Floral Printed A-Line Ethnic Set",
                "Kurta with palazzo and dupatta, dense floral print, festive wear",
                "woman", "Hourglass", "Light Brown", 0.55, 7,
            ],
        ],
        inputs=[
            model_image, garment_title, garment_detail,
            gender, body_type, skin_tone, strength, seed,
        ],
        label="Example inputs",
    )


if __name__ == "__main__":
    demo.launch()
